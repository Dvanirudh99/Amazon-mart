...
export class BaseHands {
  ...
  clickOnChildDomElementWithIndex(dom:string,index:number) {
    cy.get(`${dom}`)
      .eq(index)
      .click();
    return this;
  ...
  scrollToWithClassName(domClass: string,index:number) {
  cy.get(`.${domClass}`).eq(index).scrollIntoView();
    
    return this;
  }
  scrollWithClass(dom: string,index:number) {
...
export class BaseDependencies {
  ...
  amazonLogin(mailId:string, continueId:string, passWordID:string, signinId:string){
    cy.get(`#${mailId}`).type('dvanirudh99@gmail.com')
    cy.get(`#${continueId}`).click()
    cy.get(`#${passWordID}`).type('Anirudh19.99')
    cy.get(`#${signinId}`).click()
    return this;
  }
  addAddress(name:string, phoneNo:string, pin:string, addr: string, area: string, landmark: string, buttonId:string){
   
    cy.get(`#${name}`).type('anirudh')
    cy.get(`#${phoneNo}`).type('9177416050')
    cy.get(`#${pin}`).type('500048')
    cy.get(`#${addr}`).type('43025 4th block A-wing ')
    cy.get(`#${area}`).type('attapur, Hyderabad')
    cy.get(`#${landmark}`).type('apollo pharmacy')
